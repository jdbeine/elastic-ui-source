import {
  AfterViewChecked, ChangeDetectorRef, Component, ElementRef, OnDestroy, OnInit, QueryList, Renderer2,
  ViewChildren
} from '@angular/core';
...
import { scrollbarOptions } from '../../core/scrollbar/scrollbar.component';
    ...
    this.mailList.forEach((elem, index) => {
      this.clickListeners.push(
        this.renderer.listen(elem.nativeElement, 'click', (event) => {
          if (event.target.className !== 'mat-checkbox-inner-container mat-checkbox-inner-container-no-side-margin'
            && event.target.className !== 'star mat-icon material-icons'
            && event.target.className !== 'star active mat-icon material-icons'
            && event.target.className !== 'mat-icon material-icons') {
